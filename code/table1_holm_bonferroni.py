...
def bootstrap_r_ci(dw1, dw3, n_boot=N_BOOT):
    """CI del 95% para r (rank-biserial) via bootstrap no paramétrico."""
    rs = []
    n = len(dw1)
    a = dw1.values
    b = dw3.values
...
# --- 1. Recolectar TODAS las pruebas (lab + corridor, ambas métricas) ---
results_list = []
for scen in ["lab", "corridor"]:
    ...
    for anc in sorted(sub[COL_ANC].dropna().unique()):
        ...
        for col, label in metrics[1:]:  # Mean Excess Delay y RMS Delay Spread
            res = calculate_stats(paired[f"{col}_dw1"], paired[f"{col}_dw3"], label, scen, anc)
            if res:
                results_list.append(res)

# --- 2a. Corrección Holm-Bonferroni GLOBAL (todas las 16 pruebas a la vez) ---
...
# --- 2b. Corrección Holm-Bonferroni POR ESCENARIO (8 pruebas lab, 8 corridor, cada uno por separado) ---
for scen in ["lab", "corridor"]:
...
# --- 3. Imprimir ambas correcciones lado a lado ---
print(f"\nANCHOR-STRATIFIED TEST — Table I")
...
# --- 4. Resumen: qué combinaciones sobreviven bajo cada criterio ---
print("\n" + "="*70)
print("SUMMARY — significant (p<0.05) after correction:")
sig_global_list = [f"{r['scenario']} Anchor {r['anchor']} ({r['label']})" for r in results_list if r["p_holm_global"] < 0.05]
sig_scen_list = [f"{r['scenario']} Anchor {r['anchor']} ({r['label']})" for r in results_list if r["p_holm_scenario"] < 0.05]
